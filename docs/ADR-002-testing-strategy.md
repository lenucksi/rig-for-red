...
> Test-Strategie für die `rig_for_red` Home Assistant Custom Component.
...
```
     /\
    /  \         Manuelle / E2E-Tests (selten)
   /    \
  /------\       Integrationstests (test_coordinator.py)
 /        \
/----------\     Unit-Tests (config_flow, switch, const)
```

- **Unit-Tests:** Config-Validierung, Switch-State-Transitionen, Konstanten
- **Integrationstests:** Coordinator-Lifecycle, Dim-Logik, Schedule-Trigger, Sunrise-Restore
- **E2E:** Nicht automatisiert — manuelle Validierung mit echter HA-Instanz
